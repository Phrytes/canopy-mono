...
| **Tasks-v0 (backend)** | V2-adoption slices 1-8 shipped 2026-05-14 (embeds, crew storage policy, provisionMyCrew + /welcome.html, agent-registry, /onboard.html + /pod-settings.html, pod OIDC sign-in, multi-crew substrate enablement + spawnMyCrew, `--multi-crew` CLI, multi-crew onboarding-skill dispatch). **Tasks V2 multi-crew runtime feature-complete for web app.** Slice 9 (also 2026-05-14): **Phase 52.9.3 substrate-mirror infrastructure + addTask fan-out** — `buildTasksSubstrateStack` + `wireTasksSubstrateMirror` mirror Stoop's pattern; createCrewAgent wires per-crew mirror; addTask publishes via notifyEnvelope; receiver applies via local addItems with `source.syncedFromId` dedupe; URI-prefix filter per crewId. 3 new fan-out tests. 113/113 Tasks tests green. Pending sub-slices of 52.9.3: claim/complete/etc. fan-out (~1-2 days), stale-peer auto-heal (~0.5 day), groupCheck on fetch-resource (~0.5 day), live peer-roster updates (~0.5 day). See [`Tasks App/v2-web-functional-design-2026-05-11.md`](./Tasks%20App/v2-web-functional-design-2026-05-11.md) §6a. | ~2-3.5 days remaining |
