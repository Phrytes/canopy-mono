...
3. **Inrupt ACP integration tests against a real CSS/NSS pod** — **DONE 2026-05-16: gated test shipped + live gate-ON run executed.** Concrete negative findings (capability probe mis-detects CSS-ACP as WAC; grant→list doesn't round-trip on CSS 7.1.9 + Inrupt 3.0.0). **(a) capability probe — FIXED 2026-05-16** (`src/sharing/capabilities.js` now distinguishes CSS-ACP `.acr` from WAC `.acl`; verified verbatim, no regression). **(b) DONE 2026-05-16 (FU-b)** — root-caused: Inrupt-3.0.0 `set*Access` silent no-op vs CSS-7.1.9 ACP (never writes `.acr`); `grant`/`revoke` now throw `SHARING_*_NOOP` rather than lie. **New scoped item ⬇:** `@inrupt/solid-client` 3.0.0→current upgrade. See `Inrupt-migration/css-acp-integration-test-design-2026-05-16.md` §FOLLOW-UP (b).
   ...
   - **CORRECTED 2026-05-16: there is NO SDK upgrade.** `@inrupt/solid-client@3.0.0` IS `latest` (published 2025-11-04, maintained) — the earlier "upgrade 3.0.0→current" item was based on a wrong stale-pin assumption. The CSS-ACP no-op is a **current-vs-current interop gap**, not an old dep. Real options (see `Inrupt-migration/css-acp-integration-test-design-2026-05-16.md` §CORRECTION): **(1) accept+document** — sharing works vs Inrupt-hosted (the 52.16 target), fails loudly vs CSS [recommended, already the shipped behaviour, no code]; **(2) timeboxed spike** — confirm SDK-gap vs our-usage-error before declaring CSS unsupported; **(3)** replace sharing transport for CSS-class servers [large; only if CSS-hosted becomes a product need — ties to Stoop-browser-app + pod-provider-flexibility]. No reflexive action; user decides 1/2/3.
