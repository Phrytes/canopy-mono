...
| **Tasks-v0 (backend)** | V2-adoption slices shipped 2026-05-14 (1-8): embeds + crew storage policy → provisionMyCrew + /welcome.html → agent-registry on createCrewAgent (+ `registerAgentBundle` lifted into `@canopy/agent-registry`) → /onboard.html + /pod-settings.html → pod OIDC sign-in + listSavedCrewConfigs + /crews.html saved-crews table → multi-crew substrate enablement + spawnMyCrew + /crews.html Spawn → `bin/tasks-ui.js --multi-crew` CLI refactor → multi-crew onboarding-skill dispatch (`buildMultiCrewOnboardingSkills` resolves CrewState per-call; routes `redeemInvite` by `invite.groupId` when no crewId arg). **Tasks V2 multi-crew runtime is now feature-complete for the web app.** Audit 2026-05-14: dag.js lift (Phase 52.6.2) was already shipped earlier. Pending: Phase 52.9.3 substrate-mirror for cross-device fan-out (~3-5 days). See [`Tasks App/v2-web-functional-design-2026-05-11.md`](./Tasks%20App/v2-web-functional-design-2026-05-11.md) §6a. | ~3-5 days remaining |
