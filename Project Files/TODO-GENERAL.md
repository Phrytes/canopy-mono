...
| **Tasks-v0 (backend)** | V2-adoption slices shipped 2026-05-14: (1) `embeds:[]` on `addTask`, `crewConfig.storage`, `getCrewStoragePolicy` / `setCrewStoragePolicy`; (2) `provisionMyCrew` + `/welcome.html`; (3) agent-registry on `createCrewAgent` + `registerAgentBundle` helper lifted into `@canopy/agent-registry`; (4) `/onboard.html` + `/pod-settings.html`; (5) pod OIDC sign-in (4 skills + sign-in card) + `listSavedCrewConfigs` + `/crews.html` saved-crews table; (6) multi-crew substrate enablement (`agent` + `registerSkills` + `itemStoreRoot` opts) + `spawnMyCrew` skill + `/crews.html` Spawn button; (7) `bin/tasks-ui.js --multi-crew` CLI refactor — builds meshAgent once + crewsMap + `_spawnCrewInProcess` callback + wireSkills with `multiCrewResolver`; platform-level skills fall back to any crew when strict routing misses; 5 new multi-crew integration tests. Audit 2026-05-14: dag.js lift (Phase 52.6.2) was already shipped earlier. Known limitation: onboarding skills (`issueInvite` / `redeemInvite`) NOT registered in `--multi-crew` mode (~0.5 day to add multi-crew dispatch). Pending: Phase 52.9.3 substrate-mirror (~3-5 days). See [`Tasks App/v2-web-functional-design-2026-05-11.md`](./Tasks%20App/v2-web-functional-design-2026-05-11.md) §6a. | ~3.5-5.5 days remaining |
