...
| **Tasks-v0 (backend)** | V2-adoption slices 1-11 shipped 2026-05-14. Slices 1-8: embeds + crew storage policy + provisionMyCrew + /welcome.html + agent-registry + /onboard.html + /pod-settings.html + pod OIDC sign-in + multi-crew substrate enablement + spawnMyCrew + `--multi-crew` CLI + multi-crew onboarding-skill dispatch. **Multi-crew runtime feature-complete.** Slices 9-11: Phase 52.9.3 substrate-mirror — addTask fan-out + stale-peer auto-heal + groupCheck on fetch-resource + live peer-roster updates + mutation fan-out (`ItemStore.applySync`/`removeSync` gate-bypass + claim/complete/remove publishing). **Tasks V2 substrate-mirror feature-complete for the common-path mutations** (add/claim/complete/remove). 119/119 Tasks tests green. Remaining: submit/approve/reject/revoke/reassign publishing — clean follow-up using the same `mirror.publishTask(updated)` pattern (~0.5 day). See [`Tasks App/v2-web-functional-design-2026-05-11.md`](./Tasks%20App/v2-web-functional-design-2026-05-11.md) §6a. | ~0.5 day remaining |
