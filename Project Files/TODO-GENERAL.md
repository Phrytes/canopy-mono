...
| **Tasks-v0 (backend)** | V2-adoption slices shipped 2026-05-14: (1) `embeds:[]` on `addTask`, `crewConfig.storage`, `getCrewStoragePolicy` / `setCrewStoragePolicy`; (2) `provisionMyCrew` skill + `/welcome.html` create-crew wizard. Pending: `dag.js` lift into item-store (~1 day), agent-registry registration (~0.5 day, needs pseudoPod), `/pod-settings.html` (~1-2 days), `/onboard.html` (~1 day), multi-crew runtime, Phase 52.9.3 substrate-mirror (~3-5 days). See [`Tasks App/v2-web-functional-design-2026-05-11.md`](./Tasks%20App/v2-web-functional-design-2026-05-11.md) §6a. | ~4-6 days remaining |
