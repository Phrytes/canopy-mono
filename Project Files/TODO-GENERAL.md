...
| **Tasks-v0 (backend)** | V2-adoption slices shipped 2026-05-14: (1) `embeds:[]` on `addTask`, `crewConfig.storage`, `getCrewStoragePolicy` / `setCrewStoragePolicy`; (2) `provisionMyCrew` skill + `/welcome.html` create-crew wizard; (3) agent-registry on `createCrewAgent` (standalone pseudoPod + `registerAgentBundle` helper lifted into `@canopy/agent-registry`); (4) `/onboard.html` (invite redemption) + `/pod-settings.html` (storage-policy display + upgrade row + pod-sign-in V2.x placeholder + agent-registry status). Audit 2026-05-14: dag.js lift (Phase 52.6.2) was already shipped earlier. Pending: pod OIDC sign-in wiring (~1-2 days), multi-crew runtime, Phase 52.9.3 substrate-mirror (~3-5 days). See [`Tasks App/v2-web-functional-design-2026-05-11.md`](./Tasks%20App/v2-web-functional-design-2026-05-11.md) §6a. | ~1-2 days remaining (immediate) |
