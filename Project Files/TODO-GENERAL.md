...
| **Tasks-v0 (backend)** | V2-adoption slices 1-10 shipped 2026-05-14. Slices 1-8: embeds, crew storage policy, provisionMyCrew + /welcome.html, agent-registry, /onboard.html + /pod-settings.html, pod OIDC sign-in, multi-crew substrate enablement + spawnMyCrew + `--multi-crew` CLI + multi-crew onboarding-skill dispatch. **Multi-crew runtime feature-complete.** Slice 9: Phase 52.9.3 substrate-mirror infrastructure + addTask fan-out (buildTasksSubstrateStack + wireTasksSubstrateMirror; 3 fan-out tests). Slice 10: 52.9.3 sub-slices 2-4 — stale-peer auto-heal, groupCheck on fetch-resource, live peer-roster updates from invite redemption (3 new tests). 116/116 Tasks tests green. **Only remaining V2 item:** sub-slice 1 (claim/complete/etc. mutation fan-out) needs role-policy-gate design (~1-2 days). See [`Tasks App/v2-web-functional-design-2026-05-11.md`](./Tasks%20App/v2-web-functional-design-2026-05-11.md) §6a. | ~1-2 days remaining |
