...
| | |
|---|---|
| **Status** | Phases A + B + C all shipped.  Phase C **real-device-validated 2026-04-30** against an Inrupt-hosted pod (auth, DCR, refresh, read, write, conflict surfaces).  Polish queue still open — see "v2 queue" below. |
| **Started** | 2026-04-29 |
| **Last updated** | 2026-04-30 (late evening) — five commits today: `dbbda92` (mobile real-device validation), `bb1ab98` (412 + ENOENT regex), `e811ddd` (bg-fetch wiring v0), `2b3f427` (History tab re-added), `f40086e` (folio-serve Ctrl-C hang). |
| **Owner** | agent-folio-b (multiple) — direct + scheduled |
| **App name** | **Folio** — confirmed |
| **Blocked on** | nothing — full stack runs locally on web AND mobile |
...
| v2.5 | Force re-push + Verify-pod-state (advanced actions) | ✅ shipped (status.js + index.html `btn-force-push` lifecycle; check via grep before queuing again) |
| v2.6 | Watcher sha-stable hardening | ✅ shipped (SyncEngine `#debounceMs` + sha-stability check, "Folio v2.6 — sha-stable watcher hardening" comment) |
...
| **v2.9** | **Web UI re-shape** — Diagnostics demoted into Settings panel.  *Originally also demoted History to per-file popover only; partially reverted 2026-04-30 — top-level History tab is back AND the popover stays. Both surfaces live now (browse-everything tab + drill-into-one-file popover).* | ✅ shipped 2026-04-29 + 2026-04-30 (History tab re-added in `2b3f427`) |
...
| **v2.12** | **`folio serve` no longer hangs on Ctrl-C** — `closeAllConnections` + 4s safety-net timeout | ✅ shipped 2026-04-30 (`f40086e`) |
| ~~v2.4~~ | ~~Markdown preview toggle~~ | **dropped** (Obsidian lane) |

v2.7 + v2.8 are the "menubar-first" pieces.  v2.9 = Diagnostics demotion (History tab kept after the 2026-04-30 partial revert).  v2.12 closed a long-standing terminal-hang nuisance.
...
| | |
|---|---|
| **Status** | ✅ shipped (96aa3d7 → f500b37) — pluggable Fs/Hash/Watcher adapters; expo-file-system / expo-crypto / interval-poll watcher.  Real-device validated 2026-04-30. |
| **Tag** | [SHIPPED] |
| **Notes** | The engine layering (engine + adapter + driver) was the right call — Phase E for H7 Archive can copy the same pattern. |
...
- [x] 1. Audit Folio.A1 — list every `node:fs` / `node:path` / `chokidar` call.  These are the platform-dependent hooks.
- [x] 2. Replace with `expo-file-system` (already in mesh-demo's stack).  `chokidar` doesn't run on RN; replace with manual interval-based scan + change-detection (mtime + size).
- [ ] 3. Background-task integration — iOS `BGAppRefreshTask` via `expo-background-fetch`; Android via WorkManager (Expo wraps both).  **Foundation present (`backgroundTasks.js`) but not auto-registered.**  Pickup item.
- [x] 4. Test on RN simulator + at least one real device for each platform.  **Done on Android FP4 against Inrupt 2026-04-30.**  iOS deferred.

**DoD:**
- [x] SyncEngine runs in RN; happy-path round-trip works on simulator.
- [ ] Background sync wakes the app; pulls latest pod state.  *Pending item 3 above.*
- [x] No regressions in mesh-demo (which shares the RN package).
...
| | |
|---|---|
| **Status** | ✅ shipped (bcda197 → 9ce0c51) — Sign-in / Status / NotesList / NoteEdit / Conflicts / Share / Settings.  79 vitest tests.  **Real-device validated 2026-04-30** against Inrupt: auth + DCR + token refresh + read + write all functional. |
| **Tag** | [SHIPPED + VALIDATED] |
| **Notes** | Validation surfaced ~15 polyfill / Node-isms-in-RN traps; all fixed in commits `dbbda92` + `bb1ab98`.  Trap walkthrough + audit checklist in `apps/folio-mobile/docs/SOLID-RN-NOTES.md` — read this first before any future RN-on-Solid debugging. |
...
- [x] 1. Decide whether Folio mobile is a STANDALONE Expo app or a screen module inside mesh-demo.  **Locked: standalone (`apps/folio-mobile/`).**
- [x] 2. Notes list — tree view of pod-root mirrored locally.  Tap to edit.
- [x] 3. Note edit — plain `TextInput` editor (markdown preview deferred per C2 brief).
- [x] 4. Conflicts screen — same idea as web UI's conflict pane.
- [x] 5. Settings — WebID, pod root, sign-out.  *Pod-root form split into base + folder 2026-04-30 evening.*
- [x] 6. Tests — 79 vitest tests; manual on-device against Inrupt 2026-04-30.

**DoD:**
- [x] Notes app boots on Android (real device).  *iOS deferred — no test hardware.*
- [x] Notes editable + sync via Folio.C1 — verified end-to-end on Inrupt.
- [ ] Conflicts resolvable in-app.  *Code shipped + unit-tested; not yet exercised on a real conflict on the phone.*  Pickup item.
- [x] Repo-extraction-clean — folio-mobile lives in its own workspace, can move out cleanly.
