...
| 41.0.7 (L7) | Extend `@canopy/react-native` with `localisation/` submodule: `loadLocale({en, nl, default})` returns `{t(key, fallback), setLanguage(lang)}`. Mirrors Stoop's `localisation.js` 1:1. Stoop V3's `src/lib/localisation.js` → re-export. | `packages/react-native/src/localisation/{loadLocale.js, index.js}`, `apps/stoop-mobile/src/lib/localisation.js` |
...
| `@canopy/react-native` | + `picker/`, `mnemonic/`, `push/usePushOptIn`, `qr/`, `localisation/` submodules | ✅ additive |
...
| 41.2.6 | Locale resolver — **import `loadLocale` from `@canopy/react-native/localisation`** (lifted in 41.0 L7). Tasks-mobile passes its `apps/tasks-v0/locales/{en,nl}.json`; resolver returns `t(key)`. Wrap in a small `<LocalisationProvider>` mounted alongside `<ServiceProvider>`. | `apps/tasks-mobile/src/LocalisationProvider.js` |
