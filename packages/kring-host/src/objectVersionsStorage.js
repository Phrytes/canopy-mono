 ...
 * (γ.2 / Phase 9; repo-split W4, objective F).
 *
 ...
 *
 * The substrate `objectVersions` primitives are imported via the
 * public `@canopy/sync-engine/objectVersions` subpath export (a
 * substrate→substrate edge). The subpath — not the package barrel —
 * keeps this leaf from pulling in the `SyncEngine` (and its `chokidar`
 * fs-watcher) it doesn't need.
 */

import {
  captureObjectVersion,
  listObjectVersions,
} from '@canopy/sync-engine/objectVersions';
