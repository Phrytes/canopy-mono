...
// (SolidPodSource: core → pod-client), a dangling `import { X } from '@onderling/core'` inside the
...
describe('@onderling/pod-client barrel loads (regression guard)', () => {
  it('exports its public surface', () => {
    for (const s of ['PodClient', 'SolidPodSource', 'PodExporter', 'IdentityPodStore', 'IdentitySync', 'migrateVaultToPod']) {
      expect(podClient[s], `@onderling/pod-client must export ${s}`).toBeDefined();
    }
  });
});
