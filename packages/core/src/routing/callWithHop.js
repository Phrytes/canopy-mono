...
async function _runHopCall(agent, targetPubKey, skillId, parts, opts, outerTask) {
  ...
  // ── 5. Per bridge: prefer tunnel when advertised + plaintext ─────────────
  // Sealed mode (BB) is not yet wired through the tunnel path (CC3b). For
  // now sealed always falls back to the one-shot sealed forward.
  let lastErr;
  for (const bridge of bridges) {
    const tunnelCapable = !sealedBlob && await _bridgeSupportsTunnel(agent, bridge, opts);
    if (tunnelCapable) {
      try {
        await _openAndWireTunnel(agent, bridge, targetPubKey, skillId, parts,
                                 outerTask, originSig, originTs, opts);
        return;
      } catch (err) {
        lastErr = new Error(`tunnel via ${bridge.slice(0, 12)}… failed: ${err?.message ?? err}`);
        // Don't try one-shot through this same bridge — if the tunnel call
        // itself failed at the transport level, neither will the one-shot.
