 ...
 * from the monorepo root.  Today canopy-chat is the only app with a
 ...
 * invalid URL".  See apps/canopy-chat/playwright.config.js for the
 * per-app version (useful when running from inside that app).
 */
import { defineConfig, devices } from '@playwright/test';

export default defineConfig({
  testDir: './apps/canopy-chat/test-browser',
  fullyParallel: false,
  ...
  webServer: {
    command: 'pnpm --filter canopy-chat dev',
