...
| 27 | **`surfaces.ui.confirm`** — severity-hint gate for destructive ops (V0.8, 2026-05-20) | **`op.surfaces.ui.confirm?: { severity: 'info' \| 'warn' \| 'danger', message?: string }`** — adapter styles the confirm modal on `severity` (red for danger, yellow for warn, neutral for info) and shows `message` if present.  Surfaced by the Tier C audit (Project Files/Substrates/tier-c-proposals.md): 14 destructive surfaces across folio + tasks + stoop already have hand-rolled confirm modals; Q27 is the substrate-level signal that unlocks consistent cross-platform UX.  Out of scope: passphrase prompts + one-shot reveals (auth flows stay app-side).  Forward-additive — absent means today's behaviour (no gate). Adapters that don't understand `confirm` should fail strict, not silently ignore. |
| 28 | **`surfaces.chat.reply`** — reply-shape declaration (canopy-chat v0.1, 2026-05-21) | **`op.surfaces.chat.reply?: 'text' \| 'list' \| 'record' \| 'mini-page' \| 'file' \| 'embed-card' \| 'notification' \| 'brief'`** — the chat shell consumes this via `renderChat`'s new `replyShapeFor(opId)` lookup to pick a renderer (text bubble, list+inline-keyboard, record/mini-page panel, file attachment, embed-card, notification card, multi-section brief).  Surfaced by the canopy-chat coding plan (`Project Files/canopy-chat/coding-plan.md` § Phase v0.1): the shell needs to know WHICH renderer to call per op without hard-coding per-app heuristics.  Forward-additive — absent means "no opinion"; the shell computes a default from `verb` (mutations → `'text'`, `verb:'list'` → `'list'`) + `view.shape` (`'record'` → `'record'`).  Validator: enum-check when present; rejects unknown strings or non-strings. |
| 31 | **`surfaces.chat.followUps`** — per-op suggested-next-actions (canopy-chat v0.4, 2026-05-22) | **`op.surfaces.chat.followUps?: Array<{ opId: string, prefilledArgs?: object }>`** — after a successful dispatch the chat shell looks these up via `renderChat`'s new `followUpsFor(opId)` lookup + surfaces them as inline buttons.  Cross-app chains (`household.addMember → folio.share`) live in canopy-chat's static registry on top of this per-op hint; the manifest only declares same-app follow-ups.  Forward-additive — absent means no per-op suggestions.  Validator: rejects non-array, requires non-empty `opId` on each entry, validates `prefilledArgs` is an object when present. |
| 32 | **`op.runtime`** — runtime-environment requirement (canopy-chat v0.4, 2026-05-22) | **`op.runtime?: 'browser' \| 'node' \| 'both'`** (default `'both'`) — declares which JS runtimes the op's skill implementation supports.  Surfaced via `renderChat`'s `runtimeFor(opId)` lookup.  canopy-chat's browser-side merge filters out `runtime: 'node'` ops (folio's sync/watch family per OQ-1.A resolution); a future sidecar deployment re-includes them.  Default `'both'` means the op works anywhere.  Forward-additive — absent → `'both'`.  Validator: enum-check when present. |
