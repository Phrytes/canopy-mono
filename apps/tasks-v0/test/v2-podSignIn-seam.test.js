...
describe('podSignIn seam — backward compat (no injection)', () => {
  it('startPodSignIn builds the DEFAULT session when no sessionFactory', async () => {
    ...
    // a circle with NO oidcSession + NO sessionFactory must attempt to
    // construct one (the default path). We detect "default path
    // chosen" by pre-seeding circle.oidcSession to a fake — the module
    // must REUSE it (lazy create only when absent), proving no
    // unconditional re-creation was introduced.
    const fake = makeFakeSession();
    const circle = { dataSource: makeDataSource(), oidcSession: fake };
    const r = await startPodSignIn({
      circle, issuer: 'https://idp.example', redirectUrl: 'https://app/cb',
    });
    ...
    expect(circle.oidcSession).toBe(fake);
  });

  it('startPodSignIn still validates issuer + redirectUrl (unchanged)', async () => {
    const circle = { dataSource: makeDataSource(), oidcSession: makeFakeSession() };
    const r1 = await startPodSignIn({ circle, redirectUrl: 'https://app/cb' });
    expect(r1).toEqual({ ok: false, error: 'issuer required' });
    const r2 = await startPodSignIn({ circle, issuer: 'https://idp.example' });
  ...
  it('completePodSignIn web path uses handleCallback (callbackUrl), not adoptTokens', async () => {
    ...
    expect(circle.dataSource.hasInner).toBe(true);
  });

  it('completePodSignIn web path still rejects when no sign-in in progress', async () => {
    const circle = { dataSource: makeDataSource() };  // no oidcSession
    const r = await completePodSignIn({ circle, callbackUrl: 'https://app/cb' });
    expect(r).toEqual({
  ...
  it('signOutOfPod + podSignInStatus unchanged (duck-typed)', async () => {
    const fake = makeFakeSession();
    const circle = { dataSource: makeDataSource(), oidcSession: fake };
    expect(podSignInStatus({ circle })).toEqual({
      signedIn: true,
      webid:    'https://pod.example/alice/profile/card#me',
      podAttached: false,
    });
    const out = await signOutOfPod({ circle });
    expect(out).toEqual({ ok: true });
    expect(circle.oidcSession).toBeNull();
    expect(podSignInStatus({ circle })).toEqual({ signedIn: false });
  });
});
...
describe('podSignIn seam — injection (RN path)', () => {
  it('sessionFactory is used INSTEAD of the Node default', async () => {
    ...
    const circle = { dataSource: makeDataSource() };   // no pre-existing session
    const r = await startPodSignIn({
      circle, issuer: 'https://idp.example', redirectUrl: 'https://app/cb',
      sessionFactory,
    });
  ...
  it('completePodSignIn `tokens` path adopts tokens (no handleCallback)', async () => {
    const fake = makeFakeSession();
    const sessionFactory = vi.fn(() => fake);
    const circle = { dataSource: makeDataSource() };
  ...
  it('reuses an existing circle.oidcSession even with a sessionFactory', async () => {
    const existing = makeFakeSession();
    const factory  = vi.fn(() => makeFakeSession());
    const circle = { dataSource: makeDataSource(), oidcSession: existing };
    ...
    expect(circle.oidcSession).toBe(existing);
  });
});
