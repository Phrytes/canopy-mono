...
#
#   bash apps/basis/scripts/run-web-enclave.sh        # → http://localhost:5173
#
...
#   VITE_CIRCLE_LLM_TIMEOUT_MS=180000  bash apps/basis/scripts/run-web-enclave.sh
#   (CPU-only Ollama is ~60s/turn → the longer timeout keeps the turn from aborting.)
set -euo pipefail
cd "$(dirname "$0")/.."                       # → apps/basis
