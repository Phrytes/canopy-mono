...
describe('agents → agent-detail — same mechanism, record-shaped detail', () => {
  ...
  it('the record reply renders as a read-only key→value record (Q17)', () => {
    const el = mount();
    const record = recordFromReply({ agent: { agentId: 'laptop-anne', name: 'Anne (laptop)', status: 'active', skills: ['canopy-chat'] } });
    ...
    expect(values).toContain('["canopy-chat"]');   // nested values stay readable
  });
