...
function anonymousEnvelope() {
  ...
  return buildReportEnvelope({ records, app: 'canopy-chat', version: '1.2.3', at: 1700000000000 });
}
...
test('sink forwards the anonymous envelope intact, still anonymous, and returns ok', async () => {
  ...
  const sink = createBugReportSink({ send, target: 'fp-bugreport-dev@FAKE', app: 'canopy-chat', version: '1.2.3' });
  ...
  expect(msg.app).toBe('canopy-chat');
...
test('defensive backfill stamps only missing at/app/version', async () => {
  ...
  const sink = createBugReportSink({ send, target: 'fp-bugreport-dev@FAKE', clock: () => 999, app: 'canopy-chat', version: '9.9' });
  ...
  expect(msg.app).toBe('canopy-chat'); // backfilled (was null)
