...
For every canopy-chat web feature, this matrix records:

- The substrate skill canopy-chat dispatches to (web side).
...
Methodology: grep + read of `apps/canopy-chat/src/web/realAgent.js`
...
- **Folio-mobile is a UI stub** (#220.3) — biggest concentrated gap;
  ...
  canopy-chat-mobile under #222 carries the load).
...
- **Architecture-level deferrals** (#220.6 mesh fan-out, #220.7
  ...
  belong to canopy-chat-mobile (#222) or post-#222.
...
- Option B: defer until canopy-chat-mobile (#222) composes folio
  via the shared factory, which makes folio-mobile's wiring less
  urgent (canopy-chat-mobile becomes the unified folio entry-point).
...
**Slice fit:** rolls into #222 (canopy-chat-mobile) once #223 ships.
...
## What this tells us about #222 (canopy-chat-mobile)

The composition shell story holds up: most substrate skills are
already wired in the existing RN apps, so canopy-chat-mobile becomes
...
2. **Folio-mobile gap (#220.3)** — folio is the weakest mobile app;
   canopy-chat-mobile can become its de-facto UI (composition
...
- Reply-shape adapters in realAgent.js:1113–1280 — these are
  web-canopy-chat conventions, not mobile concerns directly. They
  belong in the `src/core/` lift under #221.5.
- Pod sign-in flows — folio-mobile and stoop-mobile both have OIDC
  flows that differ from canopy-chat-web's. Canopy-chat-mobile
