 ...
 *
 * The handler that does the heavy lifting is canopy-chat's
 * `localBuiltins.createTimeEmbed` (via callSkill('canopy-chat',
 ...
 * uses slash flags today) and canopy-chat-mobile's RN modal.
 */
...
/**
 * Submit the embed via callSkill('canopy-chat', 'embed-time', ...).
...
export async function submitEmbedTime({ state, callSkill }) {
  ...
  try {
    const result = await callSkill('canopy-chat', 'embed-time', {
