/**
 * canopy-chat — external-flow primitive (J6).
 *
 ...
 * URL (canopy-chat's own /mock-oidc page) so the round-trip is
 ...
 * URL scheme.
 *
 * Phase v0.6 sub-slice 6.2 per `/Project Files/canopy-chat/coding-plan.md`.
 */
 ...
 * canopy-chat's own callback shape (`?cc-callback=<sessionId>&...`)
...
export function parseCallbackUrl(source) {
  ...
  if (!url) return null;
  // Pattern 1: canopy-chat own callback (?cc-callback=<id>).
