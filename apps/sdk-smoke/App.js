 ...
 * Layout:
 *   ┌───────────────────────────────────┐
 *   │ Header (Init agent button)        │
 *   ├───────────────────────────────────┤
 *   │ Scenario rows (scrollable)        │  ← top half
 *   ├───────────────────────────────────┤
 *   │ Unified log pane (scrollable)     │  ← bottom half — receives
 *   │   [hh:mm:ss.mmm] [S11] line       │     output from every row
 *   └───────────────────────────────────┘
 *
...
import { LogPane }     from './src/components/LogPane.js';
...
export default function App() {
  ...
  // Unified log buffer — every scenario row appends here via `log(...)`.
  ...
  // Returns a row-scoped log fn for a given scenario id.  Each row gets
  // a stable closure so React can memoise downstream.
  const logFor = useCallback((scenarioId) => (line) => {
    setLogEntries((prev) => [...prev, { ts: Date.now(), scenarioId, line }]);
  }, []);
    ...
    <ErrorBoundary>
      <SafeAreaProvider>
        ...
        <SafeAreaView style={s.root}>
          <View style={s.layout}>
            <ScrollView style={s.scenarios} contentContainerStyle={s.scenariosContent}>
              ...
              <Text style={s.footer}>
                S1–S10 are stubs (return `pending`).  S11 is the live push-wake
                test.  See coding-plans/sdk-two-device-smoke.md.
              </Text>
            </ScrollView>
            ...
            <View style={s.logArea}>
              <LogPane entries={logEntries} onClear={clearLog} />
            </View>
          </View>
        </SafeAreaView>
      </SafeAreaProvider>
...
const s = StyleSheet.create({
  ...
  layout:         { flex: 1, paddingHorizontal: 16, paddingTop: 16 },
  scenarios:      { flex: 1 },
  scenariosContent: { paddingBottom: 16 },
  divider:        { height: 1, backgroundColor: '#1a1d27', marginVertical: 4 },
  logArea:        { flex: 1, paddingBottom: 16 },
