...
export async function handleActivate({ body, registry, provisionPod, config, now }) {
  const { projectId, code, recoveryHash, webId } = body || {};
  ...
  let result;
  try {
    result = await activate({ registry, projectId, code, recoveryHash, now: now(), provisionPod: provision, config });
  } catch (e) {
 ...
 */
export function createActivationServer({ registry, provisionPod, config, now = () => new Date().toISOString(), onRedeem }) {
  return http.createServer((req, res) => {
    ...
    req.on('end', async () => {
      ...
      const out = await handleActivate({ body: parsed, registry, provisionPod, config, now });
