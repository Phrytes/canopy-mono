...
the on-screen nav UI in canopy-chat-mobile.  Written 2026-05-26
...
- `canopyChatManifest` — re-exported from `apps/canopy-chat/src/index.js`.
  Owns chat-shell-host ops (`/help`, `/threads`, `/settings`,
  `/holiday-mode`, `/commands`, `/logs`, `/debug-dump`, …).
- `mockHouseholdManifest` — `apps/canopy-chat/src/core/agent/mockAgent.js`.
...
- `mockTasksManifest`, `mockStoopManifest`, `mockFolioManifest` —
  `apps/canopy-chat/src/core/manifests/mockManifests.js`.  Same
  pattern: declarations live in canopy-chat's core, the actual
...
`apps/canopy-chat-mobile/src/core/navModel.js`.  Walks the SAME six
...
It **does NOT know about `canopy-chat`** as an app — those ops are
...
'canopy-chat'` with a friendly "not wired on mobile yet" reply;
...
if (dispatch.appOrigin === 'canopy-chat')      // short-circuit V1
...
### "Red bubble: realAgent unknown appOrigin 'canopy-chat'"

The dispatch pipeline routed a canopy-chat-host op to the agent's
`callSkill` instead of `localBuiltins`.  Fix: intercept
`dispatch.appOrigin === 'canopy-chat'` BEFORE calling
`runDispatch` (or port the actual `localBuiltins` from
`apps/canopy-chat/src/core/localBuiltins.js` — that's #253's
later sub-step).
...
- `apps/canopy-chat-mobile/src/core/composeManifests.js`
- `apps/canopy-chat-mobile/src/core/navModel.js`
- `apps/canopy-chat-mobile/src/screens/ChatScreen.js` — the
  submitInput pipeline implementation
- `apps/canopy-chat/src/index.js` — exports
  `mergeManifests` / `parseInput` / `resolveDispatch` /
  `runDispatch` / `renderReply` / `canopyChatManifest`
- `apps/canopy-chat/web/main.js` — the canonical web pipeline
  this mobile shell mirrors
- `apps/canopy-chat/src/core/localBuiltins.js` — the
  canopy-chat-host op handlers that #253's later sub-steps will
  port to RN
