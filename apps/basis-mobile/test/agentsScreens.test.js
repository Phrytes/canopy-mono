 ...
 * (apps/basis/test/v2/agentsScreens.dom.test.js). RN rendering
...
describe('record DETAIL — the RN record-screen model (Q17, CircleRecordScreen twin)', () => {
  it('a viewAgent reply renders as read-only key→value rows', () => {
    const record = recordFromReply({ agent: { agentId: 'laptop-anne', name: 'Anne (laptop)', status: 'active', skills: ['basis'], note: null } });
    ...
    expect(byKey.skills).toBe('["basis"]');   // nested values stay readable (web parity)
